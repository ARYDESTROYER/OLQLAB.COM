# CLAUDE.md — Project conventions and working agreement
...
**Why this matters:** the journal is the only durable record of *why* a decision was made. Code shows what; commits show when; the journal shows the reasoning. Future you (or future Claude) will rely on it to avoid re-litigating settled questions.
...
1. **Stage only intended files** by name. Never `git add -A` or `git add .` — `tmp/`, `.claude/`, and unstaged scratch work shouldn't ride along.
2. **Commit message format**: short imperative subject (under 70 chars), blank line, then a paragraph or two explaining the *why* and the *what*. End with the `Co-Authored-By: Claude...` trailer when applicable.
